...
{
	public $uses=array('Systemtype','System','Chemical','ChemicalsSystem','Citation','Author','AuthorsCitation','Variable','Table');
	...
	function getsystems()
	{
		$types=$this->Systemtype->find('list',array('fields'=>array('Systemtype.id','Systemtype.sysID'),'order'=>array('Systemtype.sysID'),'limit'=>600,'offset'=>2000));
		$url=Configure::read('url.system.list');
		foreach($types as $id=>$systypeID)
		{
			$types[$id]=array('url'=>str_replace("*systypeID*",$systypeID,$url));
			...
			foreach($sysIDs[1] as $sysID)
			{
				...
				$this->System->create();
				$data=array('System'=>array('sysID'=>$sysID,'systemtype_id'=>$id,'updated'=>date(DATE_ATOM)));
	...
	function getdetails($offset="")
	{
		$systems=$this->System->find('list',array('fields'=>array('System.id','System.sysID'),'order'=>array('System.sysID'),'conditions'=>array('title'=>''),'limit'=>5));
		//echo "<pre>";print_r($systems);echo "</pre>";exit;
		
		$feedback=array();
		foreach($systems as $id=>$sysID)
		{
			$system=array('System'=>array('id'=>$id));
			$url=str_replace("*sysID*",$sysID,Configure::read('url.system.detail'));
			//echo $url."<br />";continue;
			$page=file_get_contents($url);
			list($junk,$page)=explode("NIST Standard Reference Database 106",$page);
			$page=str_replace(array(" style=\"color:#663300;font-size:12pt;\"","&nbsp;"),"",$page);
			...
			(isset($data[1][0])) ? $system['System']['title']=clean($data[1][0]) : $system['System']['title']="";
			...
			if(isset($data[1][0]))
			{
				...
				$comps=str_replace(array("NIST Chemistry WebBook for detail","[","]"),"",$comps);
			...
			preg_match_all($match,$page,$data);
			(isset($data[1][0])) ? $system['Citation']=clean($data[1][0]) : $system['citation']="";
			...
			preg_match_all($match,$page,$data);
			(isset($data[1][0])) ? $system['System']['preparer']=clean($data[1][0]) : $system['System']['preparer']="";
			...
			preg_match_all($match,$page,$data);
			(isset($data[1][0])) ? $system['System']['remarks']=clean($data[1][0]) : $system['System']['remarks']="";
			...
			foreach($tables as $tid=>$table) { $tables[$tid]=table2json($table); }
			if($tables[count($tables)-1]=="[]") { unset($tables[count($tables)-1]); }
			...
			if(isset($data[1][0]))
			{
				...
				if($notes!="") { $system['System']['datanotes']=json_encode(array_filter(explode("\t",$notes))); }
			}
			else
			{
			...
			preg_match_all($match,$page,$data);
			(isset($data[1][0])) ? $system['System']['method']=clean($data[1][0]) : $system['System']['method']="";
			...
			preg_match_all($match,$page,$data);
			(isset($data[1][0])) ? $system['System']['errors']=clean($data[1][0]) : $system['System']['errors']="";
			...
			if(isset($system['Citation'])&&$system['Citation']!="")
			{
				...
				else:						$authors=array($part[0]."."); // For only one author
				endif;
				foreach($authors as $key=>$author)
				{
					$temp=explode(", ",$author);
					$authors[$key]=array('lastname'=>$temp[0],'firstname'=>$temp[1]);
				}
				$rest=explode(", ",$part[1]); // Journal/Year => $citation[0], Volume/Issue $citation[1], pages => $citation[2]
				if(count($rest)==2) { $rest=array($rest[0],"",$rest[1]); } // Assume vol/issue is missing if only two parts
				// Journal/Year
				if($rest[0]!="")
				{
				...
				$get['aulast']=str_replace(array(",","."),"",$authors[0]);
				...
				$meta=xmlToArray($response['body']);
				$meta=$meta['crossref_result']['query_result']['body']['query'];
				$feedback[]=$meta;
				if(isset($meta['doi']['@'])) { $citation['doi']=$meta['doi']['@'];$citation["url"]="http://doi.dx.org/".$meta['doi']['@']; }
				...
				if(isset($meta['contributors']['contributor']))
				{
					...
					(!isset($meta['contributors']['contributor'][0])) ? $aus=array($meta['contributors']['contributor']) : $aus=$meta['contributors']['contributor'];
					foreach($aus as $au)
					{
						$authors[]=array('firstname'=>$au['given_name'],'lastname'=>$au['surname']);
					}
				}
				...
				{
					$result=$this->Citation->find('first',array('fields'=>array('id'),'conditions'=>array('doi'=>$citation['doi'])));
					if(empty($result))
					{
						// Add citation
						$this->Citation->create();
						if($this->Citation->save(array('Citation'=>$citation+array('updated'=>date(DATE_ATOM)))))
						{
				...
				{
					$temp=$citation;array_shift($temp); // You cannot do an array_shift inside an implode in PHP (causes error)
				...
				if(!$existing)
				{
					foreach($authors as $author)
					{
						...
						$result=$this->Author->find('first',array('fields'=>array('id'),'conditions'=>array('lastname'=>$author['lastname'],'firstname'=>$author['firstname'])));
						if(empty($result))
						{
							$this->Author->create();
							$author['updated']=date(DATE_ATOM);
							if($this->Author->save(array('Author'=>$author)))
							{
						...
						$data=array('AuthorsCitation'=>array('author_id'=>$aid,'citation_id'=>$cid,'updated'=>date(DATE_ATOM)));
			...
			// System ($needs $cid)
			if($this->System->save(array('System'=>$system['System']+array('citation_id'=>$cid))))
			{
			...
			foreach($system['Chemical'] as $chemical)
			{
				...
				($inchi!="") ? $result=$this->Chemical->find('first',array('fields'=>array('id'),'conditions'=>array('inchi'=>$inchi))) : $result=array();
				if(empty($result))
				{
					// Add to the chemicals table
					$data=array('name'=>$fields[0],'formula'=>$fields[1],'casrn'=>$fields[2],'inchi'=>$inchi,'inchikey'=>str_replace("InChIKey=","",$inchikey),'updated'=>date(DATE_ATOM));
					$this->Chemical->create();
					if($this->Chemical->save(array('Chemical'=>$data)))
					{
				...
				$data=array('ChemicalsSystem'=>array('system_id'=>$id,'chemical_id'=>$cid,'updated'=>date(DATE_ATOM)));
			...
			{
				$var=array('system_id'=>$id,'updated'=>date(DATE_ATOM));
				list($var['name'],$var['bounds'])=explode(" = ",$variable);
				$data=array('Variable'=>$var);
			...
			{
				if($table=="[]") { continue; }
				$data=array('system_id'=>$id,'content'=>$table,'updated'=>date(DATE_ATOM));
			...
			// For view
			$systems[$id]=$system;unset($system);
		}
