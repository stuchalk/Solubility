...
function clone(e,parent,rels)
{
	$(document).ready(function()
	{
		...
		children.each(function() {
			...
			if(rels) {
				var newid=oldid.replace('Rels'+oldindex,'Rels'+newindex);
			} else {
				var newid=oldid.replace(ucfirst(e)+oldindex,ucfirst(e)+newindex);
			}
			$(this).attr('id',newid);
			var oldname=$(this).attr('name');
			if(rels) {
				var newname=oldname.replace("[rels]["+oldindex+"]","[rels]["+newindex+"]");
			} else {
				var newname=oldname.replace("["+e+"]["+oldindex+"]","["+e+"]["+newindex+"]");
			}
...
function addrel(type,inputId,inputNamePrefix)
{
	$(document).ready(function()
	{
		...
		$("#"+type+index).attr(attrs1);
		$("#"+type+index).text(term);
		// Create the literal for the new term (inside div above)
		$("#"+type+index).append("<input id='"+type+index+"Literal'></input>");
		...
		$("#"+type+index).append("<input id='"+type+index+"Predicate'></input>");
