...
return this.assignSpaceGroup(sym00, ita00, bs, null, tokens, 1, trm, prevNode, sb);
...
var isUnknown = false;
var haveTransform = J.modelkit.ModelKit.isTransform(token);
...
var err = this.assignSpaceGroup(null, null, null, paramsOrUC,  Clazz.newArray(-1, [ita]), 0, null, null, sb);
...
haveUnitCell = true;
restarted = true;
}ignoreFirst = (haveUnitCell && isSetting && tokens.length > 1 && J.modelkit.ModelKit.isTransformOnly(tokens[1]));
var ita0 = (haveUnitCell ? sym00.getClegId() : null);
var trm0 = null;
if (haveUnitCell) {
if (ita0 == null || ita0.equals("0")) {
} else if (ignoreFirst) {
transform = null;
trm0 = sym00.getSpaceGroupInfoObj("itaTransform", null, false, false);
} else {
pt = ita0.indexOf(":");
if (pt > 0) {
...
if (!ignoreFirst) {
node =  new J.modelkit.ModelKit.ClegNode(index, token, transform);
...
if (tokens.length > 1) {
var msg = transform + " for " + JU.PT.join(tokens, '>', 0) + (basis.isEmpty() ? "" : "\n basis=" + basis);
...
function(token){
return (J.modelkit.ModelKit.isTransform(token) && token.indexOf(":") < 0);
}, "~S");
c$.isTransform = Clazz.defineMethod(c$, "isTransform",
function(token){
return (token.length == 0 || token.indexOf(',') > 0 || "!r!h".indexOf(token) >= 0);
}, "~S");
